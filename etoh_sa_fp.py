...
for mouse in mice:
    ...
    for date in Dates:
        ...
        #Looking at full trace with denoising
        ...
        # #plt.plot(totaltracezscore)
        # plt.eventplot(multiplied_track_licks, colors='red', lineoffsets=4, label='lick')
        # plt.eventplot(multiplied_track_lever, colors='green', lineoffsets=4, label='leverpress/ sipper out')
        # plt.eventplot(multiplied_track_leverend, colors='blue', lineoffsets=4, label='end session')
        # plt.eventplot(multiplied_track_cue, colors='pink', lineoffsets=4, label='cue')
        # plt.suptitle(f'MOUSE: {mouse}  SESSION: {Dates.index(date)}')
        # plt.legend()
        ...
        for i in range(len(track_cue)):
            ...
            cue_baseline = cue_zero + timerange[0] * fs
            cue_end = cue_zero + timerange[1] * fs
            
            zb = np.mean(df.iloc[cue_baseline:cue_zero,2])
            zsd = np.std(df.iloc[cue_baseline:cue_zero,2])
            
            # zb = np.mean(gCamp_dF_F[cue_baseline:cue_zero])
            # zsd = np.std(gCamp_dF_F[cue_baseline:cue_zero])
            baselinedict[track_cue[i]]=zb,zsd
            
            # rawtrial = np.array(gCamp_dF_F[cue_baseline:cue_end])
        ...
        # session_events=[]
        for i in range(0,10):
            fptrace_df[mouse, Dates.index(date), i]=zscore_cue[i]
            # param_thresh=0.0
            # param_thresh=0.0
            # param_prom=2
            # peaks, properties = find_peaks(zscore_cue[i], threshold = param_thresh, prominence=param_prom, height=.8)
            # plt.plot(zscore_cue[i],label=i,color=sns.color_palette("bright")[i])
            # for k in range(len(peaks)):
            #     if peaks[k] > len(zscore_cue[0])/(timerange[1]-timerange[0])*(0-timerange[0]) and peaks[k] < len(zscore_cue[0])/(timerange[1]-timerange[0])*(1.5-timerange[0]):
            #         plt.axvline(x=peaks[k], color=sns.color_palette("bright")[i], alpha=0.2)
            #         if str(i) in session_peakheight.keys():
            #             if session_peakheight[i] > zscore_cue[i][peaks[k]]:
            #                 pass
            #             else:
            #                 session_peakheight[i] = zscore_cue[i][peaks[k]]
            #         else:
            #             session_peakheight[i] = zscore_cue[i][peaks[k]]
            #         print (i, peaks[k],zscore_cue[i][peaks[k]])
            #         session_events.append(zscore_cue[i][peaks[k]])
            # plt.axvline(x=len(zscore_cue[0])/(timerange[1]-timerange[0])*(0-timerange[0]),linewidth=1, color='black')
            # plt.title(f'Mouse: {mouse}  Session: {Dates.index(date)}')
            # plt.xticks(np.arange(0,len(zscore_cue[0])+1,len(zscore_cue[0])/(timerange[1]-timerange[0])), 
        ...
        # session_peakheight_lst = session_peakheight.values()
        # cue_peakheight[mouse,Dates.index(date)]=session_peakheight_lst
        # aftercueevents[mouse,Dates.index(date)] = len(session_events)
        ...
        for length, bout in sorted_lick_bouts:
            ...
            after_zero = 1
            start_time = round(-timerange_lick[0] * fs) #with downsampling fs/N
            end_time = round((-timerange_lick[0] + after_zero) * fs) #with downsampling fs/N
...
cuepeakheightdf=pd.DataFrame.from_dict(cue_peakheight,orient='index')
############################################################
############################################################
############################################################
############################################################
...
for i in range(0,10):
    for mouse,session in avgcuetrace_dict:
        if session == i:
            plt.plot(avgcuetrace_dict[str(mouse),session],color=sns.color_palette("husl",8)[i],label=i)
plt.xticks(np.arange(0,len(zscore_cue[0])+1,len(zscore_cue[0])/(timerange[1]-timerange[0])), 
           np.arange(timerange[0], timerange[1]+1,1, dtype=int),
           rotation=0)
plt.axvline(x=len(zscore_cue[0])/(timerange[1]-timerange[0])*(0-timerange[0]),linewidth=1, color='black')
...
#find peak heights
avgpkht_df = pd.DataFrame(columns=['Mouse','Session','X-Axis','PeakHeight'])
plt.figure(figsize=(10, 6))
for mouse,session in avgcuetrace_dict:
    if session in [0,1,4,7]:
        avgtrace = avgcuetrace_dict[str(mouse),session]
        param_thresh=0.0
        param_prom=.4
        peaks, properties = find_peaks(avgtrace, threshold = param_thresh, prominence=param_prom, height=.1)
        plt.plot(avgtrace, color=sns.color_palette("husl", 8)[int(session)], label=f'Session {session}')
        for k in range(len(peaks)):
            if peaks[k] > len(avgtrace)/(timerange[1]-timerange[0])*(0-timerange[0]) and peaks[k] < len(avgtrace)/(timerange[1]-timerange[0])*(1.5-timerange[0]):
                plt.scatter(x=peaks[k], y= properties['peak_heights'][k], color=sns.color_palette("bright")[int(session)], alpha=0.2)
                num = len(avgpkht_df)
                avgpkht_df.at[num,'Mouse']=mouse
                avgpkht_df.at[num,'Session']=session
                avgpkht_df.at[num,'X-Axis']= peaks[k]
                avgpkht_df.at[num,'PeakHeight']= properties['peak_heights'][k]
                
                print (mouse, session, peaks[k], properties['peak_heights'][k])
plt.axhline(y=0, linestyle=':', color='black')
plt.xlabel('Time (samples)')
plt.xticks(np.arange(0,len(mean_trace)+1,len(mean_trace)/(timerange[1]-timerange[0])), 
           np.arange(timerange[0], timerange[1]+1,1, dtype=int),
