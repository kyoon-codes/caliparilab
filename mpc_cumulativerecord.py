...
for j in range (0,int(max(mpc_df['Session'].values))+1):
    ...
    for i in range(len(mpc_df)):
        if mpc_df.iloc[i,4] == j and mpc_df.iloc[i,2] == '1':
            list_B.append(mpc_df.iloc[i,3])
        if mpc_df.iloc[i,4] == j and mpc_df.iloc[i,2] == '2':
            lever.append(mpc_df.iloc[i,3])
        if mpc_df.iloc[i,4] == j and mpc_df.iloc[i,2] == '3':
            list_F.append(mpc_df.iloc[i,3])
        if mpc_df.iloc[i,4] == j and mpc_df.iloc[i,2] == '4':
            sucrose.append(mpc_df.iloc[i,3])
        if mpc_df.iloc[i,4] == j and mpc_df.iloc[i,2] == '5':
            licks.append(mpc_df.iloc[i,3])
    print (str(' Session: ') + str(j))
    
    cum_record = []
    reinforcement = []
    time_bin = np.arange(0,3601,60)
    ...
    for num in time_bin:
        ...
        for i in lever:
            if i < num:
                count_lever += 1
        cum_record.append(count_lever)
        for k in sucrose:
            if k < num:
                count_sucrose += 1
        reinforcement.append(count_sucrose)
    if j == 0:
        record['lever vr11'] = cum_record
        record['sucrose vr11'] = reinforcement
    if j == 1:
        record['lever fr4'] = cum_record
        record['sucrose fr4'] = reinforcement
    
    plt.figure()
    plt.eventplot([licks,sucrose,lever], lineoffsets = 2, linelengths=1, colors=['g','b','y'])
    plt.show()
