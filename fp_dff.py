...
for mouse in mice:
    ...
    for date in Dates:
        ...
        for i in range(len(track_lever)):
            ...
            avglevertrace_dict[mouse,Dates.index(date),i]=track_lever[i], sampletrial
        ...
        for i in range(len(track_flicks)):
            ...
            avgflicktrace_dict[mouse,Dates.index(date),i]=track_flicks[i], sampletrial
        ...
        for length, bout in sorted_lick_bouts:
            ...
            bout_traces[mouse, length]=(sampletrial)
            avglickbouttrace_dict[mouse,Dates.index(date),i]=start_time, sampletrial
...
fig, axs = plt.subplots(7, sharex=True, figsize=(10,12))
for i in range(7):
    for mouse,session,trial in avgcuetrace_dict:
        if mouse in ['7329']:
            if session == i:
                axs[i].plot(avgcuetrace_dict[str(mouse),session,trial][1],color=sns.color_palette("husl", 8)[int(session)],label=session)
...
for i in range(7):
    for mouse,session,trial in avglevertrace_dict:
        if mouse in ['7296']:
            if session == i:
                axs[i].plot(avglevertrace_dict[str(mouse),session,trial][1],color=sns.color_palette("husl", 8)[int(session)],label=session)
...
for i in range(7):
    for mouse,session,trial in avgflicktrace_dict:
        if mouse in ['7296']:
            if session == i:
                axs[i].plot(avgflicktrace_dict[str(mouse),session,trial][1],color=sns.color_palette("husl", 8)[int(session)],label=session)
...
avgpkht_df = pd.DataFrame(columns=['Mouse','Session','Trial','X-Axis','PeakHeight'])
    ...
    if mouse in mice:
        if session in range(8):
            if trial in range (10):
                ...
                param_thresh=0.0
                param_prom= 0.001
                peaks, properties = find_peaks(avgtrace, threshold = param_thresh, prominence=param_prom, height=0.00001)
                plt.plot(avgtrace, label=f'Session {session}')
                maxpeak=0
                for k in range(len(peaks)):
                    if peaks[k] > len(avgtrace)/(timerange_cue[1]-timerange_cue[0])*(0-timerange_cue[0]) and peaks[k] < len(avgtrace)/(timerange_cue[1]-timerange_cue[0])*(1.5-timerange_cue[0]):
                        if properties['peak_heights'][k] > maxpeak:
                            maxpeak = properties['peak_heights'][k]
                            time = peaks[k]
                            plt.scatter(x=peaks[k], y= properties['peak_heights'][k], alpha=0.2)
                
                num = len(avgpkht_df)
                avgpkht_df.at[num,'Mouse']=mouse
                avgpkht_df.at[num,'Session']=session
                avgpkht_df.at[num,'Trial']=trial
                if maxpeak > 0:
                    avgpkht_df.at[num,'X-Axis']= time
                    avgpkht_df.at[num,'PeakHeight']= maxpeak
                    allpeakheights[mouse,session,trial] = maxpeak
                else:
                    avgpkht_df.at[num,'X-Axis']= np.nan
                    avgpkht_df.at[num,'PeakHeight']= np.nan
...
for cue_mouse, cue_session, cue_trial in avglickbouttrace_dict:
    if (cue_mouse, cue_session, cue_trial) in allpeakheights.keys():
        x.append(allpeakheights[cue_mouse, cue_session, cue_trial])
        y.append(avglickbouttrace_dict[cue_mouse, cue_session, cue_trial][1])
        plt.scatter(x=allpeakheights[cue_mouse, cue_session, cue_trial], y=avglickbouttrace_dict[cue_mouse, cue_session, cue_trial][1], color=colors10[cue_session])
if len(x) > 0:
...
peakheightvlickbout = pd.DataFrame(columns=['PeakHeight','PreviousBout'])
for i in range (len(x)):
    peakheightvlickbout.at[i,'PeakHeight'] = x[i]
    peakheightvlickbout.at[i,'PreviousBout'] = y[i]
...
y=[]
for cue_mouse, cue_session, cue_trial in avglickbouttrace_dict:
    if (cue_mouse, cue_session, cue_trial+1) in allpeakheights.keys():
        x.append(allpeakheights[cue_mouse, cue_session, cue_trial+1])
        y.append(avglickbouttrace_dict[cue_mouse, cue_session, cue_trial][1])
        plt.scatter(x=allpeakheights[cue_mouse, cue_session, cue_trial+1], y=avglickbouttrace_dict[cue_mouse, cue_session, cue_trial][1], color=colors10[cue_session])
if len(x) > 0:
    a,b=np.polyfit(x,y,1)
    plt.plot(np.array(x), a*np.array(x)+b,color=colors10[i])
plt.xlabel('Peak Height')
